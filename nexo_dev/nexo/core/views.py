...
def register(request):
    ...
    messages.info(
        request,
        "O cadastro de usuários só pode ser feito pelo administrador do sistema.",
    )
...
def organograma(request):
    ...
    return render(
        request, "core/organograma.html", {"organograma_data": organograma_data_json}
    )
...
def organograma_data(request):
    ...
    try:
        ...
        unidades_com_valores = sum(
            1 for info in organograma.values() if info.get("custo_total", 0) > 0
        )
        ...
        if total_unidades > 0:
            ...
            registros_filtrados = (
                total_registros
                - UnidadeCargo.objects.exclude(grafo__exact="")
...
def simulacao_page(request):
    ...
    try:
        ...
        if os.path.exists(json_path):
            ...
            organograma_data_json = json.dumps({"core_unidadecargo": []})
    except Exception as e:
        ...
        logger = logging.getLogger(__name__)
        logger.error(
            f"Erro ao carregar dados do organograma para simulacao_page: {str(e)}"
        )

        organograma_data_json = json.dumps({"core_unidadecargo": []})

    return render(
        request, "core/simulacao.html", {"organograma_data": organograma_data_json}
    )
...
def teste_api_organograma(request):
    ...
    try:
        ...
        dados_json_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "dados.json"
        )
        ...
        dados_debug = (
            dados if dados_validos else {"error": "Formato de dados inválido ou vazio"}
        )
        ...
        if isinstance(dados_debug, dict):
            dados_debug["_debug_info"] = {
                ...
                "unidades_count": len(
                    dados.get("unidades", []) if isinstance(dados, dict) else []
                ),
        ...
        return HttpResponse(
            json.dumps(dados_debug, indent=2, ensure_ascii=False),
            content_type="application/json",
        )

    except json.JSONDecodeError as e:
        return JsonResponse(
            {"error": f"Erro ao decodificar o arquivo de dados: {str(e)}"}, status=500
        )
    except Exception as e:
        import traceback
...
def perfil(request):
    ...
    """
    return render(request, "core/perfil/perfil.html", {"perfil": request.user.perfil})
...
def editar_perfil(request):
    ...
    return render(
        request,
        "core/perfil/editar_perfil.html",
...
def alterar_senha(request):
    ...
    return render(request, "core/perfil/alterar_senha.html", {"form": form})
...
def get_unidade_data(request, codigo_unidade):
    try:
        ...
        data = {"success": False, "error": "Erro ao decodificar os dados do grafo"}
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {
                    "success": True,
                    "message": "Arquivo organograma.json atualizado com sucesso.",
                }
            )
        except Exception as e:
            return JsonResponse(
                {"success": False, "message": f"Erro ao atualizar: {str(e)}"},
                status=500,
            )
    return JsonResponse(
    ...
    try:
        ...
        with open(json_path, "r", encoding="utf-8") as file:
            ...
            for unit in units:
                ...
                if (
                    (not sigla or (unit.get("sigla", "").upper().find(sigla) >= 0))
                    and (not cargo or unit.get("tipo_cargo") == cargo)
                    and (not nivel or str(unit.get("nivel")) == nivel)
                ):
            ...
            filtered_units = [
                unit
                for unit in units
                if unit.get("codigo_unidade") in codigos_para_incluir
            ]
...
def api_cargos(request):
    ...
    try:
        ...
        # Retornar dados de fallback em caso de erro
        return JsonResponse(
            {"erro": f"Ocorreu um erro ao buscar os dados: {str(e)}", "dados": []},
            status=500,
        )
...
def api_cargos_diretos(request):
    ...
    logger.info(
        f"API cargos_diretos - Parâmetros: sigla={sigla}, tipo_cargo={tipo_cargo}, nivel={nivel}, pagina={pagina}, tamanho={tamanho}"
    )
    ...
    try:
        ...
        if tipo_cargo:
            query = query.filter(tipo_cargo=tipo_cargo)
            logger.info(
                f"Após filtro tipo_cargo={tipo_cargo}: {query.count()} registros"
            )
        ...
        if total_registros == 0:
            logger.warning(
                f"Nenhum registro encontrado com os filtros: sigla={sigla}, tipo_cargo={tipo_cargo}, nivel={nivel}"
            )
            return JsonResponse(
                {
                    "cargos": [],
        ...
        try:
            for cs in CargoSIORG.objects.all():
                ...
                if match:
                    ...
                    cargos_siorg_dict[key2] = cs
            logger.info(
                f"Carregados {len(cargos_siorg_dict)} cargos SIORG para referência"
            )
        except Exception as e:
        ...
        logger.info(
            f"Paginação: total_registros={total_registros}, total_paginas={paginator.num_pages}, pagina_atual={pagina}"
        )
            ...
            if cargo_siorg:
                ...
                if isinstance(cargo_siorg.valor, str):
                    try:
                        ...
                        logger.warning(
                            f"Erro ao converter valor '{cargo_siorg.valor}' para float"
                        )
                else:
                    valor_unitario = (
                        float(cargo_siorg.valor) if cargo_siorg.valor else 0
                    )
            ...
            result.append(
                {
                    ...
                    "sigla_unidade": cargo.sigla_unidade
                    or "",  # Campo explícito para compatibilidade
                    "tipo_unidade": cargo.tipo_unidade
                    or "",  # Campo explícito para compatibilidade
                    "tipo_cargo": cargo.tipo_cargo or "",
                    "denominacao": cargo.denominacao or "",
                    "categoria": categoria,
                    ...
                    "grafo": cargo.grafo
                    or "",  # Include grafo field for hierarchical ordering
                    "is_manual": cargo.usuario
                    is not None,  # ✅ MARCAR SE É CARGO MANUAL
                    "manual_id": (
                        cargo.id if cargo.usuario is not None else None
                    ),  # ✅ ID PARA REMOÇÃO
                }
            )
...
def comparador(request):
    ...
    try:
        ...
        organograma_data_json = json.dumps(
            {"core_unidadecargo": [], "core_cargosiorg": []}
        )

    return render(
        request, "core/comparador.html", {"organograma_data": organograma_data_json}
    )
...
def financeira_data_real(request):
    ...
    try:
        ...
        for dep in departamentos.values():
            ...
            dep["percentual"] = pct
            dep["status"] = (
                "Crítico" if pct < 50 else ("Atenção" if pct < 80 else "Adequado")
            )
        ...
        percentual = (
            (executado_total / orcamento_total * 100) if orcamento_total > 0 else 0
        )
        status = (
            "Crítico"
            if percentual < 50
...
def api_financeira_organograma(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": f"Erro ao decodificar o arquivo: {str(e)}"}, status=500
        )
    except Exception as e:
        import traceback
        ...
        traceback_str = traceback.format_exc()
        return JsonResponse(
            {"error": f"Erro inesperado: {str(e)}", "stack_trace": traceback_str},
            status=500,
        )
...
class BaixarAnexoSimulacaoView(View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            if not isinstance(estrutura_atual, list) or not isinstance(
                estrutura_nova, list
            ):
                return JsonResponse(
                    {
                        "error": "Invalid data format: estrutura_atual and estrutura_nova must be arrays."
                    },
                    status=400,
                )
            ...
            for i, item in enumerate(estrutura_atual + estrutura_nova):
                if not isinstance(item, dict):
                    return JsonResponse(
                        {"error": f"Invalid item at position {i}: must be an object."},
                        status=400,
                    )
            ...
            response["Content-Disposition"] = (
                'attachment; filename="Anexo_Simulacao.xlsx"'
            )
            return response
            ...
            if "invalid literal for int()" in str(e):
                return JsonResponse(
                    {
                        "error": "Erro de conversão de dados. Verifique se todos os campos numéricos estão preenchidos corretamente."
                    },
                    status=400,
                )
            return JsonResponse(
                {"error": str(e)}, status=400
            )  # e.g., template sheet missing or multiple active
        except Exception as e:
...
def listar_simulacoes(request):
    ...
    if tipo_usuario == "gerente":
        ...
        simulacoes = simulacoes_proprias.union(simulacoes_analise).order_by(
            "-atualizado_em"
        )
    else:
        ...
        data.append(
            {
                ...
                "usuario": (
                    sim.usuario.get_full_name() or sim.usuario.username
                    if not is_owner
                    else "Você"
                ),
                "usuario_email": sim.usuario.email if not is_owner else "",
                "is_owner": is_owner,
                "pode_enviar_analise": is_owner
                and sim.status in ["rascunho", "rejeitada", "rejeitada_editada"]
                and sim.tipo_usuario_atual == "interno",  # Usar propriedade dinâmica
                "pode_avaliar": not is_owner
                and tipo_usuario == "gerente"
                and sim.status in ["enviada_analise", "rejeitada", "rejeitada_editada"],
    ...
    response_data = {"simulacoes": data, "total": len(data), "user_type": tipo_usuario}
    ...
    if tipo_usuario == "gerente":
        ...
        print(
            f"🔍 [DEBUG] Resposta API para gerente: limite={response_data['limite']}, is_gerente={response_data['is_gerente']}"
        )
    else:
        response_data["limite"] = 5
        response_data["is_gerente"] = False
        print(
            f"🔍 [DEBUG] Resposta API para {tipo_usuario}: limite={response_data['limite']}, is_gerente={response_data['is_gerente']}"
        )
...
def salvar_simulacao(request):
    ...
    try:
        ...
        if not dados_estrutura:
            return JsonResponse(
                {"erro": "Dados da estrutura são obrigatórios"}, status=400
            )
        ...
        if simulacao_id:
            ...
            try:
                simulacao = SimulacaoSalva.objects.get(
                    id=simulacao_id, usuario=request.user
                )
                simulacao.nome = nome
                ...
                return JsonResponse(
                    {"mensagem": "Simulação atualizada com sucesso", "id": simulacao.id}
                )
            ...
            if obter_tipo_usuario(request.user) != "gerente":
                total_simulacoes = SimulacaoSalva.objects.filter(
            ...
            if SimulacaoSalva.objects.filter(usuario=request.user, nome=nome).exists():
                return JsonResponse(
                    {"erro": f'Já existe uma simulação com o nome "{nome}"'}, status=400
                )
            ...
            return JsonResponse(
                {"mensagem": "Simulação salva com sucesso", "id": simulacao.id},
                status=201,
            )
...
def carregar_simulacao(request, simulacao_id):
    ...
    try:
        ...
        return JsonResponse(
            {
                ...
                "usuario": simulacao.usuario.get_full_name()
                or simulacao.usuario.username,
        ...
        return JsonResponse(
            {"erro": "Simulação não encontrada ou sem permissão de acesso"}, status=404
        )
...
def deletar_simulacao(request, simulacao_id):
    ...
    try:
        ...
        return JsonResponse({"mensagem": f'Simulação "{nome}" deletada com sucesso'})
    except SimulacaoSalva.DoesNotExist:
        return JsonResponse({"erro": "Simulação não encontrada"}, status=404)
...
def atualizar_simulacao(request, simulacao_id):
    ...
    try:
        ...
        if simulacao.usuario != request.user:
            ...
            if user_tipo != "gerente" or simulacao.status not in [
                "enviada_analise",
                "rejeitada",
                "rejeitada_editada",
            ]:
                return JsonResponse(
                    {
                        "success": False,
                        "message": "Você não tem permissão para editar esta simulação",
                    },
                    status=403,
                )
        ...
        if "dados_estrutura" not in data:
            return JsonResponse(
                {"success": False, "message": "Dados da estrutura são obrigatórios"},
                status=400,
            )
        ...
        return JsonResponse(
            {"success": False, "message": "Dados JSON inválidos"}, status=400
        )
    except Exception as e:
        import logging
...
def relatorios(request):
    ...
    try:
        ...
        total_unidades = (
            RelatorioGratificacoes.objects.values("unidade_lotacao").distinct().count()
        )
        total_cargos = RelatorioGratificacoes.objects.values("cargo").distinct().count()

        # Dados por unidade para cálculo dos índices
        unidades_data = (
            RelatorioGratificacoes.objects.values("unidade_lotacao", "sigla_unidade")
            .annotate(
        ...
        pontos_data = (
            UnidadeCargo.objects.values("denominacao_unidade", "sigla_unidade")
            .annotate(
                total_pontos=Sum("pontos_total"), total_quantidade=Sum("quantidade")
            )
...
def api_relatorio_pontos_gratificacoes(request):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return JsonResponse(
                {"status": "error", "message": "Erro ao acessar dados do organograma"},
                status=500,
            )
...
def api_historico_decretos(request):
    ...
    try:
        ...
        return JsonResponse({"status": "success", "decretos": decretos})

    except Exception as e:
        return JsonResponse(
            {"status": "error", "message": f"Erro ao carregar decretos: {str(e)}"},
            status=500,
        )
...
def api_siglario(request):
    ...
    try:
        ...
        siglas = (
            UnidadeCargo.objects.values(
                "sigla_unidade", "denominacao_unidade", "categoria_unidade"
            )
            .distinct()
            .exclude(sigla_unidade__isnull=True)
            .exclude(sigla_unidade__exact="")
        ...
        return JsonResponse(
            {"status": "success", "data": siglario_data, "total": len(siglario_data)}
        )

    except Exception as e:
        return JsonResponse(
            {"status": "error", "message": f"Erro ao carregar siglário: {str(e)}"},
            status=500,
        )
...
def api_unidades_disponiveis(request):
    ...
    try:
        unidades = (
            UnidadeCargo.objects.values("denominacao_unidade", "sigla_unidade")
            .distinct()
            .exclude(denominacao_unidade__isnull=True)
            .exclude(denominacao_unidade__exact="")
        ...
        return JsonResponse({"status": "success", "unidades": unidades_data})

    except Exception as e:
        return JsonResponse(
            {"status": "error", "message": f"Erro ao carregar unidades: {str(e)}"},
            status=500,
        )
...
def enviar_solicitacao_realocacao(request):
    ...
    try:
        ...
        return JsonResponse(
            {"status": "error", "message": f"Erro ao processar solicitação: {str(e)}"},
            status=500,
        )
...
def listar_simulacoes_gerente(request):
    ...
    for sim in simulacoes:
        ...
        return JsonResponse({"simulacoes": data, "total": len(data)})
...
def enviar_simulacao_para_analise(request):
    ...
    try:
        ...
        if obter_tipo_usuario(request.user) != "interno":
            return JsonResponse(
                {
                    "erro": "Apenas usuários internos podem enviar simulações para análise"
                },
                status=403,
            )
        ...
        if simulacao.status not in ["rascunho", "rejeitada", "rejeitada_editada"]:
            return JsonResponse(
                {"erro": "Esta simulação já foi enviada para análise ou aprovada"},
                status=400,
            )
        ...
        gerentes = User.objects.filter(
            tipo_usuario_simulacao__tipo="gerente", tipo_usuario_simulacao__ativo=True
        )
        ...
        if status_anterior in ["rejeitada", "rejeitada_editada"]:
            mensagem_sucesso = (
                "Simulação corrigida e reenviada para análise com sucesso"
            )
        else:
            mensagem_sucesso = "Simulação enviada para análise com sucesso"

        return JsonResponse(
            {"mensagem": mensagem_sucesso, "status": simulacao.get_status_display()}
        )
...
def avaliar_simulacao(request):
    ...
    try:
        ...
        if not all([simulacao_id, acao]):
            return JsonResponse(
                {"erro": "ID da simulação e ação são obrigatórios"}, status=400
            )

        if acao not in ["aprovar", "rejeitar"]:
            return JsonResponse(
                {"erro": 'Ação deve ser "aprovar" ou "rejeitar"'}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"erro": "Simulação não encontrada ou não disponível para avaliação"},
                status=404,
            )
...
def criar_solicitacao_simulacao(request):
    ...
    try:
        ...
        if obter_tipo_usuario(request.user) != "gerente":
            return JsonResponse(
                {"erro": "Apenas gerentes podem criar solicitações"}, status=403
            )
        ...
        if not all([usuario_designado_id, titulo, descricao]):
            return JsonResponse(
                {"erro": "Usuário designado, título e descrição são obrigatórios"},
                status=400,
            )
        ...
        try:
            ...
            if obter_tipo_usuario(usuario_designado) != "interno":
                return JsonResponse(
                    {"erro": "Usuário designado deve ser um usuário interno"},
                    status=400,
                )
        except User.DoesNotExist:
            return JsonResponse(
                {"erro": "Usuário designado não encontrado"}, status=404
            )
        ...
        if prazo_estimado:
            try:
                ...
                return JsonResponse(
                    {"erro": "Formato de data inválido para prazo estimado"}, status=400
                )
        ...
        data.append(
            {
                ...
                "aceita_em": (
                    sol.aceita_em.strftime("%d/%m/%Y %H:%M") if sol.aceita_em else ""
                ),
                "simulacao_criada_id": (
                    sol.simulacao_criada.id if sol.simulacao_criada else None
                ),
                "simulacao_criada_nome": (
                    sol.simulacao_criada.nome if sol.simulacao_criada else ""
                ),
                "observacoes_usuario": sol.observacoes_usuario or "",
            }
        )

    return JsonResponse({"solicitacoes": data, "total": len(data)})
    ...
    try:
        ...
        try:
            solicitacao = SolicitacaoSimulacao.objects.get(
                id=solicitacao_id, usuario_designado=request.user, status="pendente"
            )
        except SolicitacaoSimulacao.DoesNotExist:
            return JsonResponse(
                {"erro": "Solicitação não encontrada ou já foi processada"}, status=404
            )
...
def vincular_simulacao_solicitacao(request):
    ...
    try:
        ...
        if not all([solicitacao_id, simulacao_id]):
            return JsonResponse(
                {"erro": "ID da solicitação e da simulação são obrigatórios"},
                status=400,
            )
        ...
        try:
            solicitacao = SolicitacaoSimulacao.objects.get(
                id=solicitacao_id, usuario_designado=request.user
            )
            simulacao = SimulacaoSalva.objects.get(
                id=simulacao_id, usuario=request.user
            )
            ...
            return JsonResponse(
                {"erro": "Solicitação ou simulação não encontrada"}, status=404
            )
...
def minhas_notificacoes(request):
    ...
    """
    notificacoes = NotificacaoSimulacao.objects.filter(usuario=request.user).order_by(
        "-criada_em"
    )[
        :20
    ...
    return JsonResponse(
        {"notificacoes": data, "total": len(data), "nao_lidas": nao_lidas}
    )
...
def excluir_todas_notificacoes(request):
    ...
    try:
        ...
        if total_notificacoes == 0:
            return JsonResponse(
                {"mensagem": "Nenhuma notificação encontrada para excluir"}
            )

        # Excluir todas as notificações do usuário
        NotificacaoSimulacao.objects.filter(usuario=request.user).delete()
...
def adicionar_cargo(request):
    ...
    try:
        ...
        if categoria < 1 or categoria > 4:
            return JsonResponse(
                {"success": False, "error": "Categoria deve estar entre 1 e 4"},
                status=400,
            )

        if nivel < 1 or nivel > 18:
            return JsonResponse(
                {"success": False, "error": "Nível deve estar entre 1 e 18"}, status=400
            )

        if quantidade < 1:
            return JsonResponse(
                {"success": False, "error": "Quantidade deve ser pelo menos 1"},
                status=400,
            )
        ...
        return JsonResponse(
            {"success": False, "error": f"Erro de validação: {str(e)}"}, status=400
        )
...
def mesclar_simulacoes(request):
    ...
    try:
        ...
        if obter_tipo_usuario(request.user) != "gerente":
            return JsonResponse(
                {"erro": "Acesso negado. Apenas gerentes podem mesclar simulações."},
                status=403,
            )
        ...
        if not simulacao_ids or len(simulacao_ids) < 2:
            return JsonResponse(
                {"erro": "Selecione pelo menos 2 simulações para mesclar."}, status=400
            )

        if not nome_nova_simulacao:
            return JsonResponse(
                {"erro": "Nome da nova simulação é obrigatório."}, status=400
            )
        ...
        # Verificar se já existe simulação com mesmo nome
        if SimulacaoSalva.objects.filter(
            usuario=request.user, nome=nome_nova_simulacao
        ).exists():
            return JsonResponse(
                {"erro": f'Já existe uma simulação com o nome "{nome_nova_simulacao}"'},
                status=400,
            )
        ...
        for sim_id in simulacao_ids:
            try:
                ...
                return JsonResponse(
                    {"erro": f"Simulação {sim_id} não encontrada ou sem acesso."},
                    status=404,
                )
        ...
        if simulacoes_nao_aprovadas:
            nomes_nao_aprovadas = [
                f'"{s["nome"]}" ({s["status"]})' for s in simulacoes_nao_aprovadas
            ]
            return JsonResponse(
                {
        ...
        if not todos_dados:
            return JsonResponse(
                {"erro": "Nenhum dado encontrado nas simulações selecionadas."},
                status=400,
            )
        ...
        if metodo_mesclagem == "somar":
            ...
            for item in todos_dados:
                ...
                if chave in grupos:
                    # Somar quantidades
                    grupos[chave]["quantidade"] = int(
                        grupos[chave].get("quantidade", 0)
        ...
        unidade_base_final = (
            unidades_base[0] if unidades_base else "Simulações Mescladas"
        )
        ...
        print(
            f"🔗 [DEBUG] Simulação mesclada criada: {nova_simulacao.nome} (ID: {nova_simulacao.id}) com {len(dados_mesclados)} itens usando método '{metodo_mesclagem}' de {len(simulacoes)} simulações aprovadas"
        )
