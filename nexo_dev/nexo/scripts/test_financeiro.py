...
class FinanceiroTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.plano_receitas = PlanoContas.objects.create(
            codigo="1", descricao="Receitas", tipo="R"  # Receita
        )

        self.plano_despesas = PlanoContas.objects.create(
            codigo="2", descricao="Despesas", tipo="D"  # Despesa
        )
    ...
    def test_1_criar_lancamentos_receitas(self):
        ...
        self.assertEqual(
            LancamentoFinanceiro.objects.filter(categoria=self.cat_vendas).count(), 1
        )
        self.assertEqual(
            LancamentoFinanceiro.objects.filter(categoria=self.cat_servicos).count(), 1
        )
    ...
    def test_2_criar_lancamentos_despesas(self):
        ...
        self.assertEqual(
            LancamentoFinanceiro.objects.filter(categoria=self.cat_impostos).count(), 1
        )
        self.assertEqual(
            LancamentoFinanceiro.objects.filter(categoria=self.cat_folha).count(), 1
        )
    ...
    def test_6_gerar_fluxo_caixa(self):
        ...
        def gerar_fluxo_caixa(data_inicial, data_final):
            fluxo = {
                ...
                "receitas": {"total": 0, "categorias": {}},
                "despesas": {"total": 0, "categorias": {}},
            ...
            receitas = LancamentoFinanceiro.objects.filter(
                tipo="R", status="PG", data__gte=data_inicial, data__lte=data_final
            )
            ...
            despesas = LancamentoFinanceiro.objects.filter(
                tipo="D", status="PG", data__gte=data_inicial, data__lte=data_final
            )
    ...
    def test_7_gerar_dre(self):
        ...
        def gerar_dre(data_inicial, data_final):
            ...
            receitas = LancamentoFinanceiro.objects.filter(
                tipo="R", data__gte=data_inicial, data__lte=data_final
            )
            ...
            dre["resultado_operacional"] = (
                dre["lucro_bruto"] - dre["despesas_operacionais"]
            )
    ...
    def test_9_transferencia_entre_contas(self):
        ...
        # Verificar saldos
        self.assertEqual(
            saldo_principal, self.conta_principal.saldo_inicial - valor_transferencia
        )
        self.assertEqual(
            saldo_investimentos,
            self.conta_investimentos.saldo_inicial + valor_transferencia,
        )
