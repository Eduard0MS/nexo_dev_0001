...
def run_test(script_path):
    ...
    try:
        result = subprocess.run(
            [sys.executable, full_path], capture_output=True, text=True
        )
        ...
        if result.returncode == 0:
            logger.info(
                f"Teste concluído com sucesso: {script_path} (tempo: {execution_time:.2f}s)"
            )
        ...
        execution_time = end_time - start_time
        logger.error(
            f"Erro ao executar teste {script_path}: {str(e)} (tempo: {execution_time:.2f}s)"
        )
...
def run_all_tests():
    ...
    report_path = os.path.join(
        BASE_DIR,
        "logs",
        f"test_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Executa todos os testes do sistema Nexo"
    )
    parser.add_argument(
        "--skip",
        type=str,
        help="Lista de scripts a serem ignorados, separados por vírgula",
    )
    ...
    if args.skip:
        ...
        TEST_SCRIPTS = [
            script for script in TEST_SCRIPTS if script not in scripts_to_skip
        ]
