...
def exportar_csv_simples(dados, componente="completo"):
    ...
    response['Content-Disposition'] = f'attachment; filename="financeiro_{componente}_backup.csv"'
    ...
    writer.writerow(['Orçamento Total', f'R$ {dados.get("orcamento_total", 0):.2f}'.replace('.', ',')])
    writer.writerow(['Executado Total', f'R$ {dados.get("executado_total", 0):.2f}'.replace('.', ',')])
...
def exportar_html_simples(dados, componente="completo"):
    ...
    response['Content-Disposition'] = f'inline; filename="financeiro_{componente}_backup.html"'
    return response 