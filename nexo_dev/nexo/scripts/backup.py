...
def copy_to_external(file_path):
    ...
    if not os.path.exists(BACKUP_EXTERNAL_DIR):
        logger.warning(
            f"Diretório externo não encontrado: {BACKUP_EXTERNAL_DIR}. Pulando backup externo."
        )
        return
...
def copy_to_cloud(file_path):
    ...
    if not os.path.exists(BACKUP_CLOUD_DIR):
        logger.warning(
            f"Diretório de nuvem não encontrado: {BACKUP_CLOUD_DIR}. Pulando backup na nuvem."
        )
        return
...
def main():
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Executa um backup completo (DB, config, dados)",
    )
    parser.add_argument(
        "--db", action="store_true", help="Executa apenas o backup do banco de dados"
    )
    parser.add_argument(
        "--config", action="store_true", help="Executa apenas o backup de configurações"
    )
    parser.add_argument(
        "--userdata",
        action="store_true",
        help="Executa apenas o backup de dados de usuário",
    )
    ...
    try:
        ...
        if do_full or args.config:
            ...
            config_backup_enc = (
                encrypt_file(config_backup) if ENCRYPTION_ENABLED else config_backup
            )
        ...
        if do_full or args.userdata:
            ...
            userdata_backup_enc = (
                encrypt_file(userdata_backup) if ENCRYPTION_ENABLED else userdata_backup
            )
