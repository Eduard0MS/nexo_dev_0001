...
def test_ransom_note():
    ...
    for name in ransom_note_names:
        ...
        with open(file_path, 'w') as f:
            ...
            f.write("Para recuperar seus arquivos, você precisa pagar um resgate de 1 Bitcoin.\n")
            f.write("Entre em contato com hack3r@malici0us-domain.com para instruções de pagamento.\n")
    ...
    logger.info("Verifique logs/ransomware_monitor.log para ver se as notas foram detectadas")
...
def main():
    ...
    parser.add_argument('--mass-changes', action='store_true', help='Testa detecção de alterações em massa')
    parser.add_argument('--extension', action='store_true', help='Testa detecção de alterações de extensão')
    parser.add_argument('--ransom-note', action='store_true', help='Testa detecção de notas de resgate')
    parser.add_argument('--no-cleanup', action='store_true', help='Não remove os arquivos de teste após a execução')
