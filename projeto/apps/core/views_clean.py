    ...
    try:
        ...
        if periodo == 'dia':
            ...
            for i in range(10):
                ...
                orcado.append(round(orcamento_base * 0.033 * (0.9 + 0.2 * random.random()), 2))
                realizado.append(round(executado_base * 0.033 * (0.85 + 0.3 * random.random()), 2))
        elif periodo == 'semana':
            # Últimas 8 semanas
            for i in range(8):
                labels.append(f"Semana {i+1}")
                orcado.append(round(orcamento_base * 0.25 * (0.9 + 0.2 * random.random()), 2))
                realizado.append(round(executado_base * 0.25 * (0.85 + 0.3 * random.random()), 2))
        elif periodo == 'mes':
            ...
            for i in range(7):
                ...
                if i < 6:  # Meses passados
                    orcado.append(round(orcamento_base * (0.9 + 0.2 * random.random()), 2))
                    realizado.append(round(executado_base * (0.85 + 0.3 * random.random()), 2))
                else:  # Mês projetado
                    orcado.append(round(orcamento_base * 1.05, 2))
                    realizado.append(0)  # Valor zero para projeção, mais seguro que None
        elif periodo == 'ano':
            ...
            for i in range(6):
                ...
                if i < 5:  # Anos passados
                    orcado.append(round(orcamento_base * 12 * (0.9 + 0.2 * random.random()), 2))
                    realizado.append(round(executado_base * 12 * (0.85 + 0.3 * random.random()), 2))
                else:  # Ano projetado
            ...
            for i in range(12):
                ...
                orcado.append(round(orcamento_base * 3.5 * (0.9 + 0.2 * random.random()) / 12, 2))
                realizado.append(round(executado_base * 3.5 * (0.85 + 0.3 * random.random()) / 12, 2))
        ...
        for i in range(len(labels)):
            ...
            valor_realizado = realizado[i] if i < len(realizado) and realizado[i] is not None else 0
            
            execucao_mensal.append({
                "mes": labels[i],
                "orcado": valor_orcado,
                "executado": valor_realizado
            })
        ...
        dados = {
            ...
            "status": status,
            "variacao_periodo": 12.5 if periodo == 'mes' else 8.7,  # Simulação de variação
...
def api_organograma(request):
    ...
    try:
        ...
        unidades = UnidadeCargo.objects.select_related('unidade').exclude(grafo__exact='').exclude(grafo__isnull=True)
        ...
        for grafo, grupo_unidades in unidades_por_grafo.items():
            ...
            for unidade in grupo_unidades:
                try:
                    ...
                    valor = Decimal(cargo_siorg.valor.replace('R$ ', '').replace('.', '').replace(',', '.'))
                    ...
                    print(f"Cargo não encontrado: {unidade.tipo_cargo} {unidade.categoria} {unidade.nivel:02d}")
        ...
        def construir_arvore(grafo):
            ...
            if grafo in hierarquia:
                ...
                for filho in filhos:
                    ...
                    unidade['valores']['pontos_total'] += filho['valores']['pontos_total']
