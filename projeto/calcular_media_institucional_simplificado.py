...
import sys
import os
# Adicionar o diretório atual ao path para encontrar o módulo config
...
def calcular_media_institucional_simplificado():
    ...
    # Obter todas as siglas únicas de UnidadeCargo (apenas como referência)
    ...
    # Contar TODOS os funcionários da tabela RelatorioGratificacoes (534 funcionários)
    unidades_funcionarios = defaultdict(int)
    
    # Contar funcionários por coordenação diretamente da tabela
    for registro in RelatorioGratificacoes.objects.all():
        coordenacao = registro.coordenacao
        
        # Ignorar registros com coordenacao nula ou vazia
        if coordenacao is None or coordenacao == "":
            continue
        
        # Contar apenas funcionários com coordenação válida
        unidades_funcionarios[coordenacao] += 1
    
    total_funcionarios_contados = sum(unidades_funcionarios.values())
    
    # Mostrar contagem por coordenação
    for coordenacao, count in sorted(unidades_funcionarios.items()):
        print(f"  - {coordenacao}: {count} funcionários")
    
    print(f"✅ Contados {total_funcionarios_contados} funcionários para {len(unidades_funcionarios)} unidades")
    ...
    print(f"Total de coordenações únicas em RelatorioGratificacoes: {len(unidades_funcionarios)}")
    print(f"Total de funcionários contados: {total_funcionarios_contados}")
    ...
    return {
        ...
        'correspondencias_exatas': total_funcionarios_contados
    }
