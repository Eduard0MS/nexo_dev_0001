...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--nome", type=str, required=True, help="Nome para identificar o relatório"
        )
        parser.add_argument(
            "--descricao", type=str, default="", help="Descrição opcional do relatório"
        )
        parser.add_argument(
            "--usuario",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f'Usuário "{username}" não encontrado. Criando relatório sem usuário associado.'
                )
            )
            usuario = None
        ...
        try:
            ...
            self.stdout.write(
                f"Relatório criado: {relatorio.nome} (ID: {relatorio.id})"
            )
        ...
        self.stdout.write(
            f"Acesse o admin em: /admin/core/relatorio/{relatorio.id}/change/"
        )

        if relatorio.processado:
            self.stdout.write(
                f"Estatísticas em: /admin/core/relatorio/{relatorio.id}/estatisticas/"
            )
