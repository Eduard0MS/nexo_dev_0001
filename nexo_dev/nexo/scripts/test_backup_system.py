...
def test_backup_integrity():
    ...
    if integrity_failures == 0:
        logger.info(
            "Todos os arquivos de backup passaram na verificação de integridade"
        )
        return True
    else:
        logger.error(
            f"{integrity_failures} arquivo(s) falharam na verificação de integridade"
        )
        return False
...
def test_restore_backup():
    ...
    for root, _, files in os.walk(BACKUP_DIR):
        for file in files:
            if file.startswith("database_") and (
                file.endswith(".gz") or file.endswith(".gpg")
            ):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            # Para arquivo .gpg (criptografado)
                ...
                logger.info(
                    "Backup criptografado encontrado (a descriptografia requer chave)"
                )
...
def main():
    try:
        ...
        logger.info(
            f"Integridade dos backups: {'SUCESSO' if integrity_result else 'FALHA'}"
        )
        logger.info(
            f"Restauração de backup: {'SUCESSO' if restore_result else 'FALHA'}"
        )

        if backup_result and integrity_result and restore_result:
            logger.info(
                "TODOS OS TESTES PASSARAM - Sistema de backup funcionando corretamente"
            )
            return 0
        else:
