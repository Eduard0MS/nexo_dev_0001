...
class Command(BaseCommand):
    ...
    def _create_documentacao(self, options):
        ...
        self.stdout.write(self.style.WARNING("📝 Criando nova documentação..."))
        ...
        self.stdout.write(self.style.SUCCESS("✅ Documentação criada com sucesso!"))

    def _read_documentacao(self, options):
        """READ: Visualiza documentação existente."""
        if not self.build_path.exists():
            raise CommandError(
                "Documentação não encontrada. Execute com --acao create primeiro."
            )
    ...
    def _update_documentacao(self, options):
        ...
        self.stdout.write(self.style.WARNING("🔄 Atualizando documentação..."))
        ...
        if options["watch"]:
            ...
            self.stdout.write(self.style.SUCCESS("✅ Documentação atualizada!"))

    def _delete_documentacao(self, options):
        """DELETE: Remove arquivos de documentação."""
        self.stdout.write(self.style.WARNING("🗑️  Removendo documentação..."))

        if self.build_path.exists():
            shutil.rmtree(self.build_path)
            self.stdout.write(self.style.SUCCESS("✅ Arquivos de build removidos"))
    ...
    def _gerar_doc_modelos(self):
        ...
        conteudo = self._renderizar_template(
            "auto_models.rst", {"modelos": modelos, "data_geracao": "now"}
        )
    ...
    def _gerar_doc_views(self):
        ...
        conteudo = self._renderizar_template(
            "auto_views.rst", {"views": views_info, "data_geracao": "now"}
        )
    ...
    def _gerar_doc_forms(self):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING("⚠️  Módulo forms não encontrado"))
    ...
    def _gerar_doc_utils(self):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING("⚠️  Módulo utils não encontrado"))
    ...
    def _compilar_documentacao(self, formato="html"):
        ...
        if sys.platform == "win32":
            try:
                ...
                if resultado.returncode == 0:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"✅ Documentação compilada em: {self.build_path / formato}"
                        )
                    )
        ...
        try:
            ...
            raise CommandError(
                "sphinx-build não encontrado. Verifique a instalação do Sphinx."
            )
    ...
    def _abrir_documentacao(self, servidor=False, port=8000):
        ...
        if servidor:
            ...
            if not html_path.exists():
                self.stdout.write(
                    self.style.WARNING(
                        "⚠️ Documentação não encontrada. Gerando primeiro..."
                    )
                )
            ...
            try:
                ...
                if "Address already in use" in str(e):
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ Porta {port} já está em uso. Tente outra porta."
                        )
                    )
